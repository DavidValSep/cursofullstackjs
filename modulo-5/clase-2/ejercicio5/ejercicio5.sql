...
SELECT * FROM bandas WHERE pais != 'Alemania' AND nombre IN (SELECT nombre_banda FROM bandas_discos WHERE anio_disco >= 2000);

--2. Listar el disco más reciente de las bandas inglesas que terminan en ‘s’.
SELECT DISTINCT ON (nombre_banda) nombre_banda, nombre_disco, anio_disco FROM bandas_discos WHERE nombre_disco LIKE '%s' ORDER BY nombre_banda, anio_disco DESC;

--3. Listar todas las bandas alemanas con al menos un letra K en su nombre que tengan discos publicados en 1999 o superior
SELECT * FROM bandas WHERE pais = 'Alemania' AND (nombre LIKE '%k%' OR nombre LIKE 'k%' OR nombre LIKE '%k') AND nombre IN (SELECT nombre_banda FROM bandas_discos WHERE anio_disco >= 1999);
...
SELECT ban.nombre, dis.anio_disco FROM bandas ban JOIN bandas_discos dis ON ban.nombre = dis.nombre_banda ORDER BY dis.anio_disco

--6. Listar todas las bandas que tienen un disco con nombre empezado en A. Listar el nombre de la banda y del disco
SELECT ban.nombre, dis.nombre_disco FROM bandas ban JOIN bandas_discos dis ON ban.nombre = dis.nombre_banda AND dis.nombre_disco LIKE 'A%'

--7. Listar todas las bandas que tengan discos con más de una palabra.
--Listar el nombre de la banda y del disco.
SELECT nombre, nombre_disco FROM bandas ban JOIN bandas_discos dis ON ban.nombre = dis.nombre_banda AND dis.nombre_disco LIKE '% %'

--8. Listar todas las bandas que tengan discos con más de una palabra.
--Listar el nombre de la banda y la cantidad de discos.
SELECT ban.nombre, COUNT(dis.nombre_disco) AS cantidad_discos
FROM bandas ban
JOIN bandas_discos dis ON ban.nombre = dis.nombre_banda
WHERE dis.nombre_disco LIKE '% %'
GROUP BY ban.nombre;